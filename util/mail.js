...
module.exports = (messageData) => {
  ...
  if (messageData.company.length > 0)
    messageText += ` with ${messageData.company}`;
  messageText += `</strong>\n
    <p><strong>Message</strong>: ${messageData.message.replace(
      /(<([^>]+)>)/gi,
      ""
    )}
    <p><em><a href="mailto:${
      messageData.email
    }?subject=Re:${messageData.subject.replace(
    / /g,
    "%20"
  )}">Click to reply</a></em></p>
    `;

  const mailOptions = {
    from: messageData.email,
    to: "mike@mikepawlak.com",
